...
class JSONLQdrantProcessor:
    ...
    def chunk_and_embed(self, records: List[Dict[str, Any]]) -> List[PointStruct]:
        ...
        for record_idx, record in enumerate(records):
            if 'section' not in record or 'content' not in record:
                logger.warning(f"Record {record_idx} missing required fields (section/content). Skipping.")
                continue
            ...
            if not content.strip():
                logger.warning(f"Record {record_idx} has empty content. Skipping.")
                continue
            
            # Split content into chunks
            chunks = self.text_splitter.split_text(content)
            logger.info(f"Section '{section}': Split into {len(chunks)} chunks")
            ...
            for chunk_idx, chunk in enumerate(chunks):
                ...
                point = PointStruct(
                    ...
                    payload={
                        ...
                        "original_record_index": record_idx,
